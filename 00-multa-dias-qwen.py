...
def salvar_dados():
    ...
    dados = {
        ...
        "label_data_despacho": st.session_state.get("label_data_despacho", "Data da ciência da decisão"),
        "mostrar_linhas_calculo": st.session_state.get("mostrar_linhas_calculo", {
            "data_despacho": True,
            "prazo_cumprimento": True,
            "fim_prazo": True,
            "inicio_multa": True
        }),
        "faixas": [
...
def carregar_dados(dados_codificados):
    ...
    try:
        ...
        st.session_state.label_data_despacho = dados.get("label_data_despacho", "Data da ciência da decisão")
        st.session_state.mostrar_linhas_calculo = dados.get("mostrar_linhas_calculo", {
            "data_despacho": True, "prazo_cumprimento": True, "fim_prazo": True, "inicio_multa": True
        })
...
def limpar_dados():
    ...
    st.session_state.mostrar_linhas_calculo = {
        "data_despacho": True, "prazo_cumprimento": True, "fim_prazo": True, "inicio_multa": True
    }
...
def gerar_preview_html(res, mostrar_linhas, label_data_despacho):
    """Gera uma visualização HTML do relatório para preview no Streamlit"""
    html = f"""
    <div style="font-family: Arial, sans-serif; font-size: 11px; line-height: 1.6; border: 1px solid #ccc; padding: 15px; background: #fff;">
        <h3 style="text-align: center; margin-bottom: 15px;">Relatório de Multa Diária Corrigida</h3>
        
        <p><strong>Número do Processo:</strong> {st.session_state.get('proc_input', '---')}</p>
        <p><strong>Autor:</strong> {st.session_state.get('autor_input', '---')}</p>
        <p><strong>Réu:</strong> {st.session_state.get('reu_input', '---')}</p>
        
        <hr style="margin: 15px 0;">
        
        <p><strong>Cálculo do Início da Multa:</strong></p>
    """
    
    linhas = [
        ("data_despacho", f"{label_data_despacho}:", res['data_despacho'].strftime('%d/%m/%Y')),
        ("prazo_cumprimento", "Prazo para cumprimento:", f"{res['prazo_cumprimento']} {res['tipo_prazo'].lower()}"),
        ("fim_prazo", "Fim do prazo:", res['data_fim_prazo'].strftime('%d/%m/%Y')),
        ("inicio_multa", "Início da multa:", res['data_inicio_multa'].strftime('%d/%m/%Y'))
    ]
    
    for key, label, valor in linhas:
        if mostrar_linhas.get(key, True):
            html += f'<p style="margin: 2px 0;"><span style="display: inline-block; width: 200px;">{label}</span><strong>{valor}</strong></p>\n'
    
    html += '<hr style="margin: 15px 0;"><p><strong>Detalhamento das Faixas:</strong></p>\n'
    ...
    for i, faixa in enumerate(st.session_state.faixas):
        ...
        html += f'<p style="margin: 3px 0;">Faixa {i+1}: {faixa["inicio"].strftime("%d/%m/%Y")} a {faixa["fim"].strftime("%d/%m/%Y")} | {dias_contabilizados} {tipo_dias} | Valor: {moeda_br(faixa["valor"])}/dia | Total: {moeda_br(dias_contabilizados * faixa["valor"])}</p>\n'
    
    html += f"""
    <hr style="margin: 15px 0;">
    <p><strong>Atualização da multa:</strong></p>
    <p style="margin: 2px 0;"><span style="display: inline-block; width: 200px;">Data de atualização:</span><strong>{res['data_atualizacao'].strftime('%d/%m/%Y')}</strong></p>
    <p style="margin: 2px 0;"><span style="display: inline-block; width: 200px;">Total de dias em atraso:</span><strong>{res['total_dias']}</strong></p>
    <p style="margin: 2px 0;"><span style="display: inline-block; width: 200px;">Multa sem correção:</span><strong>{moeda_br(res['total_sem_correcao'])}</strong></p>
    
    <hr style="margin: 15px 0;">
    <p><strong>Correção mês a mês:</strong></p>
    """
    ...
    for mes in res["meses_ordenados"]:
        ...
        html += f'<p style="margin: 2px 0;">{data_formatada}: {moeda_br(bruto)} x {indice*100:.2f}% = {moeda_br(corrigido)}</p>\n'
    
    html += f"""
    <hr style="margin: 15px 0;">
    <p><strong>Multa corrigida:</strong> {moeda_br(res['total_corrigido'])}</p>
    """
    
    observacao = st.session_state.get("obs_input", "").strip()
    if observacao:
        fonte = st.session_state.get("fonte_obs", "Arial")
        tam = st.session_state.get("tam_obs", 8)
        html += f'<p style="margin-top: 10px; font-style: italic; font-size: {tam}px; font-family: {fonte};"><strong>Observação:</strong> {observacao}</p>\n'
    
    html += """
    <hr style="margin: 15px 0;">
    <p style="font-size: 9px; font-style: italic;">Nota: A correção foi realizada com base na taxa SELIC acumulada, conforme fatores disponíveis no site do CJF.</p>
    <p style="font-size: 9px;">Documento é assinado e datado eletronicamente.</p>
    </div>
    """
    return html
...
def gerar_pdf(res, numero_processo, nome_autor, nome_reu, observacao=None, 
              ...
              mostrar_linhas=None):
    """Gera o PDF com opção de ocultar linhas do cálculo"""
    if mostrar_linhas is None:
        mostrar_linhas = {"data_despacho": True, "prazo_cumprimento": True, "fim_prazo": True, "inicio_multa": True}
...
""", unsafe_allow_html=True)
    ...
    if "indices_selic" not in st.session_state:
        st.session_state.indices_selic = {}
    if "label_data_despacho" not in st.session_state:
        st.session_state.label_data_despacho = "Data da ciência da decisão"
    if "mostrar_linhas_calculo" not in st.session_state:  # ← NOVO: controle de visibilidade
        st.session_state.mostrar_linhas_calculo = {
            "data_despacho": True, "prazo_cumprimento": True, "fim_prazo": True, "inicio_multa": True
        }

    # Bloco de campos dinâmicos
    ...
    if "resultado_multa" in st.session_state:
        ...
        # ← NOVO: Preview do relatório com controles de visibilidade
        st.markdown("---")
        st.subheader("👁️ Preview do Relatório")
        
        with st.expander("⚙️ Personalizar linhas do 'Cálculo do Início da Multa'", expanded=False):
            st.markdown("**Marque/desmarque as linhas que deseja exibir no relatório:**")
            col_chk1, col_chk2 = st.columns(2)
            with col_chk1:
                st.session_state.mostrar_linhas_calculo["data_despacho"] = st.checkbox(
                    "✓ " + label_data_despacho,
                    value=st.session_state.mostrar_linhas_calculo.get("data_despacho", True),
                    key="chk_data_despacho"
                )
                st.session_state.mostrar_linhas_calculo["fim_prazo"] = st.checkbox(
                    "✓ Fim do prazo",
                    value=st.session_state.mostrar_linhas_calculo.get("fim_prazo", True),
                    key="chk_fim_prazo"
                )
            with col_chk2:
                st.session_state.mostrar_linhas_calculo["prazo_cumprimento"] = st.checkbox(
                    "✓ Prazo para cumprimento",
                    value=st.session_state.mostrar_linhas_calculo.get("prazo_cumprimento", True),
                    key="chk_prazo"
                )
                st.session_state.mostrar_linhas_calculo["inicio_multa"] = st.checkbox(
                    "✓ Início da multa",
                    value=st.session_state.mostrar_linhas_calculo.get("inicio_multa", True),
                    key="chk_inicio_multa"
                )
            st.info("💡 As linhas ocultas podem ser detalhadas na caixa de 'Observações' abaixo.")
        
        # Renderiza o preview em HTML
        preview_html = gerar_preview_html(res, st.session_state.mostrar_linhas_calculo, label_data_despacho)
        st.markdown(preview_html, unsafe_allow_html=True)
        
        # Detalhamento por mês (tabela)
        detalhamento = []
        for mes in res["meses_ordenados"]:
            bruto = res["totais_mensais"][mes]
            indice = res["indices"].get(mes, 0.0)
        ...
        with st.expander("📄 Gerar Relatório PDF", expanded=True):
            ...
            with col2:
                observacao = st.text_area("Observações", height=405, key="obs_input", 
                                         help="Aqui você pode detalhar manualmente as informações que optou por ocultar acima.")
                    ...
                    with st.spinner("Gerando documento..."):
                        try:
                            pdf_data = gerar_pdf(
                                ...
                                st.session_state.mostrar_linhas_calculo  # ← NOVO: passa as preferências de visibilidade
                            )
                            if pdf_data:
                                st.download_button(
                                    "⬇️ Baixar PDF",
