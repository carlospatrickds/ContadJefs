...
def processar_dados(df):
    ...
    if processed_df['data_chegada_obj'].isna().all() and 'DIAS_TRANSCORRIDOS' in processed_df.columns:
        ...
        def calcular_data_chegada_painel_gerencial(row):
            ...
            try:
                # CORREÇÃO SOLICITADA:
                # Usar a fórmula: (HOJE() - 1) - QUANTIDADE_DE_DIAS_EM_ATRASO
                dias = int(dias_transcorridos)
                data_referencia_painel = data_referencia - timedelta(days=1)  # HOJE() - 1
...
# --- Funções de Estatísticas, Relatórios e Download (MODIFICADAS) ---
...
def criar_relatorio_filtros(df_filtrado, filtros_aplicados):
    ...
    if len(df_filtrado) > 0:
        # CORREÇÃO 1: Ajustar larguras das colunas para mostrar número completo do processo
        pdf.set_font('Arial', 'B', 9)
        colunas = ['Nº Processo', 'Polo Ativo', 'Data', 'Servidor', 'Assunto']
        # CORREÇÃO: Aumentar largura da coluna Nº Processo para mostrar número completo
        larguras = [50, 40, 20, 25, 55]  # Aumentei Nº Processo de 35 para 50
        ...
        # CORREÇÃO 2: Ordenar do mais antigo para o mais recente
        if 'Data Chegada' in df_filtrado.columns:
            # Converter para datetime para ordenação correta
            df_filtrado_temp = df_filtrado.copy()
            df_filtrado_temp['Data Chegada DT'] = pd.to_datetime(
                df_filtrado_temp['Data Chegada'], 
                format='%d/%m/%Y', 
                errors='coerce'
            )
            df_filtrado_temp = df_filtrado_temp.sort_values('Data Chegada DT', ascending=True)  # Mais antigo primeiro
            df_ordenado = df_filtrado_temp.drop('Data Chegada DT', axis=1)
        else:
            df_ordenado = df_filtrado
        
        # CORREÇÃO 3: Destacar processos com mais de 90 dias
        data_hoje = get_local_time().date()
        ...
        for _, row in df_ordenado.iterrows():
            ...
            # Verificar se tem mais de 90 dias
            try:
                if data_chegada:
                    data_chegada_dt = datetime.strptime(data_chegada, '%d/%m/%Y').date()
                    dias_atraso = (data_hoje - data_chegada_dt).days
                    mais_90_dias = dias_atraso > 90
                else:
                    mais_90_dias = False
            except:
                mais_90_dias = False
            
            # Aplicar destaque para processos com mais de 90 dias
            if mais_90_dias:
                pdf.set_fill_color(255, 200, 200)  # Fundo vermelho claro
                pdf.set_font('Arial', 'B', 7)  # Negrito para destaque
            else:
                pdf.set_fill_color(255, 255, 255)  # Fundo branco
                pdf.set_font('Arial', '', 7)  # Fonte normal
            
            # CORREÇÃO: Mostrar número completo do processo sem truncar
            pdf.cell(larguras[0], 8, n_processo, 1, 0, 'L', fill=True)
            pdf.cell(larguras[1], 8, polo_ativo[:20], 1, 0, 'L', fill=True)
            pdf.cell(larguras[2], 8, data_chegada[:10], 1, 0, 'C', fill=True)
            pdf.cell(larguras[3], 8, servidor[:12], 1, 0, 'L', fill=True)
            pdf.cell(larguras[4], 8, assunto[:35], 1, 0, 'L', fill=True)
            pdf.ln()
            
            # Resetar cor e fonte para a próxima linha
...
def main():
    ...
    if uploaded_files:
        ...
        # -- ADIÇÃO: Detectar se algum arquivo é do Painel Gerencial (apenas coluna DIAS_TRANSCORRIDOS)
        ...
        with tab4:
            ...
            # MOSTRAR TODOS OS PROCESSOS, não apenas os sem etiqueta
            todos_processos = processed_df.copy()
            
            # Filtros para agrupar por assunto
            ...
            with col1:
                ...
                assunto_filtro = st.selectbox(
                    "Filtrar por Assunto para Agrupar:",
                    options=["Todos"] + assuntos_disponiveis,
                    key='filtro_assunto_atribuicao'
                )
            ...
            with col2:
                ...
                status_filtro = st.selectbox(
                    "Filtrar por Status:",
                    options=["Todos", "Sem Atribuição", "Com Atribuição"],
                    key='filtro_status_atribuicao'
                )
                
                if status_filtro == "Sem Atribuição":
                    processos_filtrados = processos_filtrados[
                        processos_filtrados['servidor'].isin(["Sem etiqueta", "Não atribuído"])
                    ]
            ...
            with col1:
                ...
                if not processos_filtrados.empty:
                    # Ordenar por data de chegada (mais antigos primeiro para prioridade)
                    processos_para_atribuir = processos_filtrados.sort_values(
                        by=['data_chegada_obj', 'NUMERO_PROCESSO'], 
                        ascending=[True, True]
                    ).copy()
                    
                    # Exibir todos os processos filtrados
                    cols_to_show = [
                        'NUMERO_PROCESSO', 'POLO_PASSIVO', 'vara', 'data_chegada_formatada', 'DIAS', 'ASSUNTO_PRINCIPAL', 'servidor'
                    ]
