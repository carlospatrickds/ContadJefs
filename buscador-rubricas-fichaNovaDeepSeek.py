...
class ExtratorDemonstrativos:
    ...
    def processar_pdf(self, pdf_file, extrair_proventos: bool = True, extrair_descontos: bool = True) -> pd.DataFrame:
        """Processa o PDF usando análise de Tabela (mais robusta) com Fallback em texto."""
        dados = []
        ...
        with pdfplumber.open(pdf_file) as pdf:
            for pagina_num, pagina in enumerate(pdf.pages, 1):
                ...
                # ABORDAGEM 1: Extração por tabela (ideal para SIAPE, resolve colunas/meses vazios)
                tabelas = pagina.extract_tables()
                tabela_processada = False
                
                if tabelas:
                    for tabela in tabelas:
                        # Limpa células da tabela
                        tabela_limpa = [[str(c).strip().replace('\n', ' ') if c is not None else '' for c in linha] for linha in tabela]
                        
                        idx_cabecalho = -1
                        for i, linha in enumerate(tabela_limpa):
                            linha_upper = [c.upper() for c in linha]
                            # Reconhece linha de meses se bater 3 meses ou ter 'DISCRIMINAÇÃO'
                            if sum(1 for m in self.meses_ordenados if m in linha_upper) >= 3 or any("DISCRIMINA" in c for c in linha_upper):
                                idx_cabecalho = i
                                break
                                
                        if idx_cabecalho >= 0:
                            tabela_processada = True
                            cabecalho = tabela_limpa[idx_cabecalho]
                            
                            # Mapear os índices exatos de cada mês
                            meses_indices = {}
                            ...
                            # Processar as rubricas abaixo do cabeçalho
                            for linha in tabela_limpa[idx_cabecalho+1:]:
                                if not any(linha): continue  # Ignora linhas puramente vazias
                                
                                col_0 = linha[0].upper() if len(linha) > 0 else ""
                                col_1 = linha[1].upper() if len(linha) > 1 else ""
                                
                                # Verifica quebras de seções
                                if "RENDIMENTO" in col_0 or "RENDIMENTO" in col_1:
                                    secao_atual = "RENDIMENTO"
                                elif "DESCONTO" in col_0 or "DESCONTO" in col_1:
                                    secao_atual = "DESCONTO"
                                    
                                # Nas tabelas SIAPE: 1º coluna = TIPO, 2º coluna = DISCRIMINAÇÃO (mas pode estar mesclada)
                                nome_rubrica = linha[1] if len(linha) > 1 and linha[1].strip() else col_0
                                
                                nome_rubrica_upper = nome_rubrica.upper()
                                if nome_rubrica_upper in ["RENDIMENTOS", "DESCONTOS", "TIPO", ""] or "DISCRIMINA" in nome_rubrica_upper:
                                    continue
                                    
                                if not secao_atual:
                                    continue
                                    
                                if (secao_atual == "RENDIMENTO" and not extrair_proventos) or \
                                   (secao_atual == "DESCONTO" and not extrair_descontos):
                                    continue
                                
                                # Coletar os valores respeitando exatamente a coluna de cada mês
                                for mes_abbr, idx in meses_indices.items():
                                    if idx < len(linha):
                                        valor_str = linha[idx]
                                        valor_float = self.converter_valor_string(valor_str)
                                        # Registra apenas se a rubrica tem desconto/provento naquele mês
                ...
                # ABORDAGEM 2: FALLBACK PARA TEXTO (se a tabela for invisível)
                                ...
                                while pos < len(linha_upper):
                                    ...
                                    for m in self.meses_ordenados:
                                        if linha_upper.startswith(m, pos):
                                            ...
                                            break
                                    if not match: pos += 1
                                meses_pagina = encontrados[:6]
                            break
                            
                    if cabecalho_idx is not None and meses_pagina:
                        for linha in linhas[cabecalho_idx+1:]:
                            ...
                            if linha_upper.startswith("RENDIMENTOS"):
                                secao_atual = "RENDIMENTO"
                                resto = linha[11:].strip()
                                if resto and extrair_proventos:
                                    self._processar_linha_rubrica(resto, secao_atual, meses_pagina, ano, pagina_num, dados)
                            elif linha_upper.startswith("DESCONTOS"):
                                secao_atual = "DESCONTO"
                                resto = linha[9:].strip()
                                if resto and extrair_descontos:
                                    self._processar_linha_rubrica(resto, secao_atual, meses_pagina, ano, pagina_num, dados)
                            elif secao_atual:
                                if (secao_atual == "RENDIMENTO" and extrair_proventos) or \
                                   (secao_atual == "DESCONTO" and extrair_descontos):
        ...
        # Opcional: Remover registros com valor zero absoluto da exportação final
        df = df[df['Valor'] != '0,00']
        
        # Adicionar numeração sequencial para rubricas duplicadas na mesma competência (Ex: VENCIMENTO BASICO #1)
    ...
    def _processar_linha_rubrica(self, linha, secao, meses, ano, pagina, dados):
        """Processa uma linha de rubrica via texto (Fallback) corrigido."""
        padrao_num = r'(?:R\$\s*)?(\d{1,3}(?:\.\d{3})*,\d{2})'
        ...
        # Lidar com seções agrupadas indevidamente ao texto
        if nome_rubrica.upper().endswith("RENDIMENTOS"):
            nome_rubrica = nome_rubrica[:-11].strip()
        elif nome_rubrica.upper().endswith("DESCONTOS"):
            nome_rubrica = nome_rubrica[:-9].strip()
            
        if not nome_rubrica:
            return
            
        parte_numeros = linha[primeiro_num.start():]
        numeros = re.findall(padrao_num, parte_numeros)
        
        # Não usamos mais o "if len(numeros) < len(meses): return", pois ele descartava
        # linhas com meses em branco incorretamente pelo processo de texto.
        limite = min(len(numeros), len(meses))
