...
def corrigir_texto(txt: str) -> str:
    """
    Corrige problemas clássicos de mojibake:
    OLÃ© -> OLÉ
    """
    if not txt:
        return ""
...
def extrair_dados(pdf_bytes: bytes) -> pd.DataFrame:
    ...
    # 1️⃣ Lê TODAS as páginas e junta em um único texto contínuo
    ...
    while i < len(linhas):
        ...
        # 4️⃣ Detecta cabeçalho com meses
...
if pdf_file:
    ...
    if df.empty:
        ...
        st.download_button(
            "📥 Baixar CSV",
            data=csv,
            file_name="extracao_siape_completa.csv",
            mime="text/csv"
        )
